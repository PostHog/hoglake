...
| `DELETE` | DONE | puffin DV merge + superseding registration; multi-statement transactions merge per data_file_id (one live DV per file per commit); same-transaction scans mask buffered deletes (`hoglake_txn.test`); duplicate-path registrations get per-copy position attribution via the rowid base (rowid predicates grow only the matching copy's DV) and inconsistent/out-of-range external metadata refuses typed (`hoglake_wire_hardening.test`); conflict = re-run (no blind retry) |
