...
| `hog_column` | `catalog_id`, `table_id`, `field_id`, `begin_snapshot`, `end_snapshot`, `name`, `col_type`, `type_params` (jsonb), `nullable`, `ordinal`, `parent_field_id` | `col_type` CHECK (closed 26-type set, every member Iceberg-mappable; `list`/`struct`/`map` are containers); identifier CHECK; `ordinal >= 0` CHECK + PER-PARENT live-ordinal partial unique, `NULLS NOT DISTINCT` so top-level columns (NULL parent) keep the guarantee (duplicate live ordinal = parquet-writer corruption vector, refused by the DB); `parent_field_id <> field_id` CHECK | Versioned column TREE; `field_id` is the stable parquet binding (`PARQUET:field_id`); `parent_field_id` is a same-table reference by `(catalog_id, table_id, field_id)` identity and deliberately NOT an FK — these rows are versioned, so an FK would name one version of a parent that a rename retires. `ordinal` orders SIBLINGS. |
...
| `hog_data_file` | `catalog_id`, `data_file_id`, `table_id`, `begin/end_snapshot`, `path`, `file_format`, `record_count`, `file_size_bytes`, `footer_size`, `row_id_start`, `stats_state`, `spec_id`, `explicit_row_ids`, `missing_field_ids`, `split_offsets` | PK; `stats_state` CHECK (`provided\|pending\|failed`); count/size CHECKs; live + pending partial indexes | The manifest. `row_id_start` NOT NULL (lineage guarantee); `explicit_row_ids` marks compaction outputs carrying `_hog_row_id`; `missing_field_ids` flags id-less files (rename guard); `split_offsets` (`bigint[]`, V18, nullable) holds each row group's first-column-chunk start offset for row-group-aligned splits (GET /scan `include=split_offsets`) |
...
| `hog_file_removal` | `removal_id` (identity PK), `catalog_id`, `path`, `file_kind`, `reason`, `scheduled_at`, `attempts`, `last_attempt_at`, `drained_at`, `drained_outcome` | `reason` CHECK (`snapshot_expiry\|table_drop_gc\|compaction_staging`); `drained_outcome` CHECK (`deleted\|absent\|registered`) paired-null with `drained_at`; two partial indexes over the UNDRAINED rows — `(catalog_id, removal_id)` for the drain's ordered LIMIT, `(catalog_id, path)` for the commit path-reuse guard (V16); the path one is non-unique on purpose (one path can be queued twice) | Physical-deletion queue AND forensics ledger: drains soft-delete (outcome + timestamp survive), skips/transients bump `attempts`; `compaction_staging` rows are the compactor's output-path claim tickets, settled `'registered'` on group commit |
| `hog_maintenance_run` | `run_id` (identity PK), `catalog_id`, `task`, `run_trigger`, `started_at`, `finished_at`, `status`, `error`, `result` (jsonb) | `task` CHECK (`hydrator\|expiry\|cleanup\|compaction\|verify`); `run_trigger` CHECK (`loop\|manual`); `status` CHECK (`ok\|failed`) paired with `error`; (catalog, task, run_id DESC) index | The maintenance run ledger (V2): one row per task run, loop sweep or manual trigger, recorded after the run resolves (never inside its transaction); `result` is the matching POST response body verbatim. Hydrator sweeps record per claimed catalog. Purged by the cleanup sweep past `HOGLAKE_MAINTENANCE_LEDGER_RETENTION_SECONDS` (7d default); backs `GET /maintenance/status` + `/runs` |

| `hog_maintenance_summary` | `catalog_id`, generation pointers, `sampled_at`, `sample`, `scan_state`, `next_batch_at` | FK CASCADE; due-work index; sample/timestamp paired-null | Published dashboard snapshots and durable bounded-scan checkpoints; cold state is unknown, not zero |
| `hog_maintenance_summary_tier` | catalog/generation/bucket key, table/spec/partition values, quota/remainder/pending, selected/file/byte/DV counts | PK (catalog, generation, bounded bucket hash); FK CASCADE | Per-bucket accumulators carried across scan pages; published generations back the partition report |
