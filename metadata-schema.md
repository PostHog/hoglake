...
| `hog_maintenance_summary_tier` | catalog/generation/bucket key, table/spec/partition values, quota/remainder/pending, selected/file/byte/DV counts | PK (catalog, generation, bounded bucket hash); FK CASCADE | Per-tier accumulators carried across scan pages; published generations back the partition report |
